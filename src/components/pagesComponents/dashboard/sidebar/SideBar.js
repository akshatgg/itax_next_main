...
export default function SideBar({ data, setIsNavigating, topOffset }) {
  const [activeItem, setActiveItem] = useState(null);
  const [isExpanded, setIsExpanded] = useState(false); // now controls open/close
  ...
  return (
    ...
    >
      {/* Top Toggle Section */}
      <div className="relative w-full px-4 py-3 border-b border-gray-200 flex justify-center">
        <button onClick={toggleSidebar}>
          <Icon
            icon={isExpanded ? "ant-design:menu-fold-outlined" : "ant-design:menu-unfold-outlined"}
            className="text-xl text-blue-600 hover:text-blue-800 transition-colors duration-200"
          />
        </button>
      </div>
        ...
        <ul className="py-2">
          {data?.map((item, i) => (
            <SideBarItem
              ...
              activeItem={activeItem}
              setIsNavigating={setIsNavigating}
              isSidebarOpen={isExpanded} // pass expanded state instead of hover
            />
          ))}
        </ul>
      </nav>
