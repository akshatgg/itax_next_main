...
const salesData = {
  ...
  body: [], // dynamic sales data goes here
};
...
export default function AllSales() {
  ...
  const filteredSales = useMemo(() => {
    if (!searchTerm) return salesData.body;
    const term = searchTerm.toLowerCase();
    return salesData.body.filter((item) => item.item_name.toLowerCase().includes(term));
  }, [searchTerm]);
    ...
    <section className="p-4 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap gap-2 justify-end mb-6">
        ...
        <Link href="/dashboard/accounts/invoice/Cash_&_Bank">
          <button className={`${buttonClasses} bg-green-600 text-white hover:bg-green-700`}>
            Cash & Bank
          </button>
        </Link>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-6">
        <div className="relative flex-1">
          ...
          />
          <Icon icon="zondicons:search" className="absolute right-3 top-1/2 -translate-y-1/2 text-neutral-400" />
        </div>
        <select
          value={filter}
